...
最终自动化测试已完成，Sites 归档大小待精确部署包生成后回填：
...
本轮新的私有部署尚未完成，因此不得沿用旧版本信息作为当前证明：
...
1. 先运行本交接第 10 节的全部验证命令，回填最终测试数量和最终 Sites 归档大小。
2. 只提交经过验证的精确源码与治理产物。
3. 部署新的所有者私有 Sites 版本。
4. 抽检首页、三类专题、两个旗舰案例、资讯、联系页、`robots.txt` 和 `sitemap.xml`。
5. 回填提交号、Sites 版本、私有地址与部署状态。
6. 之后只按企业签核结果逐条放行，不批量推定同类内容通过。
7. 公开前重新取得用户明确批准。
