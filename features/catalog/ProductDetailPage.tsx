...
export function ProductDetailPage({ product }: { product: ProductRecord }) {
  ...
  const schema = { "@context": "https://schema.org", "@type": "Product", name: product.title, sku: product.source_id, mpn: product.model, model: product.model, category: product.topic, image: product.gallery, brand: { "@type": "Brand", name: "钜豪照明 JUHAO" }, url: `${SITE_URL}${product.seo_slug}`, description: `${product.topic}产品，查看结构化参数、安装提示和相关方案。` };
  return <main id="main-content" className={styles.page}>
    <section className={styles.hero}>
      ...
      <div className={styles.summary}>
        ...
        <dl><div><dt>产品型号</dt><dd>{product.model}</dd></div><div><dt>产品专题</dt><dd>{product.topic}</dd></div><div><dt>资料状态</dt><dd>{product.sale_status} · 已通过机器门禁，当前为私有预览</dd></div></dl>
