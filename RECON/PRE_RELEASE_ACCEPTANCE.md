...
- 移动端首屏启用 `viewport-fit=cover`，Hero 从页面顶部开始；导航使用深色半透明底并把安全区包含在导航内部，390px 下 Logo 与菜单保持清晰对比。
- 全站浮动操作层提供三类咨询方向选择器与返回顶部：返回顶部只在达到滚动阈值后进入可见和键盘序列，联系页不显示自链接。路由变化使用 JUHAO 幕布过场，并以会话中的上一路径兼容客户端导航和整页重载。
- 首页首屏、业务段、页尾、导航、悬浮入口与 Footer 已统一为家庭健康光、工程项目、渠道合作三条咨询路径；链接携带 `source`、`scene`、`intent`，联系页服务端预选方向、提示语和隐藏上下文字段。
...
三档浏览器实际覆盖：首页首屏切换、390px 顶部安全区与导航对比度、家庭/工程/渠道三类咨询预设与来源字段、资讯轮播、桌面/移动导航及搜索、移动抽屉焦点闭环、分组 Footer、产品 dialog、服务地区查询与 FAQ、智能场景、合作区域选择、联系预检、关键词搜索、资讯第 1→2→3 页的客户端导航，以及悬浮咨询、返回顶部、路由幕布和 reduced-motion 降级。
