...
- 首页加 23 个独立内页，共 24 个访客页面，均服务端输出单一 `h1`、可读正文和自指 canonical。
- 当前 sitemap 发布 13 个可索引 URL。另有 11 个待核验或工具页面设置为 `noindex, follow`，且不输出页面级 `WebPage`、`Service`、`FAQPage` 或面包屑结构化数据。
...
- 旧 `/brand/**`、`/healthy`、`/esg`、`/investment`、`/download`、`/law` 和数字新闻详情均为 308 迁移；已取证的 `/news/page/1` 至 `/3` 明确迁移到 `/news`，未知页码保持 404。
...
- 桌面导航支持 hover/focus 二级菜单；移动端支持抽屉、子菜单、Esc、滚动锁和焦点可达。
- 五类照明场景均有独立入口；商业场景具备 Mock 产品加载、空态、错误重试、产品原生 dialog、焦点回送和可见 FAQ。
- 服务页具备地区/城市联动、查询、加载/空态/错误/结果和 FAQ；合作页具备区域选择、选中状态、说明面板及错误重试。
- 智能家居具备完整 Tab 键盘模型；搜索、新闻列表和下载均通过 `SiteApi`，覆盖加载、错误重试、空态与成功态；联系页只做本地信息完整度预检，不提交任何数据。
- `prefers-reduced-motion` 下停止自动播放和 GSAP/WebGL 动效，正文、静态图片与全部核心操作仍可用。
...
| ESLint | 通过 |
| production build | 通过 |
| Node SSR/SEO 测试 | 6/6 通过 |
| 三档浏览器交互 | 1440×900、768×1024、390×844 通过 |
| 24 路由 DOM/SEO/基础无障碍巡检 | 1440×900 与 390×844 共 48 次页面检查通过 |
| 控制台错误 / page error / HTTP ≥400 | 0 / 0 / 0（已测正常流程） |
| reduced-motion | WebGL 隐藏、静态背景可见、自动播放控制移除 |

三档浏览器实际覆盖：首页首屏切换、资讯轮播、桌面/移动导航、产品 dialog、服务地区查询与 FAQ、智能场景、合作区域选择、联系预检和关键词搜索。
...
1. 尚未提供钜豪目标 CMS/API 的地址、鉴权方式与字段契约；产品、门店、合作区域、搜索、新闻与下载仍由统一 `SiteApi` Mock adapter 提供。
2. 历史、招聘、商城能力、服务网点、合作政策、ESG 数据、下载文件、联系方式、法律与隐私文本仍需企业一手资料审核。
3. 联系页没有后端提交通道；下载页没有已核验文件；因此相关页面保持 `noindex`。
