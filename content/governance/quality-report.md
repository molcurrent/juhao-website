# 产品内容台账质量报告

生成日期：2026-07-13

## 数据范围

- 商城 SQL 用于核对 `isSale`、`goodsStatus`、`dataFlag`、库存与交期。
- 企业知识库商品说明用于核对事业部、结构化参数、主图和详情图。
- 仅把 10 个首批专题中通过机器门禁的产品写入私有预览详情页数据。

## 结果

- 统一发布台账：163 条记录，其中 95 条当前私有预览路由、0 条可索引路由。
- 路由覆盖：58 条静态/功能路由，31 条产品详情，6 条案例详情。
- 待治理：151 条未登记审核人，142 条未完成媒体公开授权确认。
- 审核台账：99 款候选，保持每个专题 10 款。
- 私有预览：31 款；产品 ID 唯一，无跨专题重复发布。人工审核与媒体授权完成前不进入公开 SEO。
- 深专题发布上限：射灯、家居顶灯各 6 款；其余专题各 3 款。
- 参数完整度：最低 84%，平均 99.5%。
- 在售门禁：`isSale=1`、`goodsStatus=1`、`dataFlag=1`。
- 图片门禁：至少 4 张，且全部来自企业商城 OSS 渠道。
- 智能设备专题因缺少结构化参数，当前没有产品详情进入首批私有预览范围。

## 首批产品分布

- 射灯：6 款
- 家居顶灯：6 款
- 新中式：3 款
- 艺术灯：3 款
- 水晶吊灯：3 款
- 灯带：3 款
- 开关面板：1 款
- 户外照明：3 款
- 工程定制：3 款
- 家居智能设备：0 款

## 发布说明

库存与交期只用于后台核验，不作为官网实时承诺；预览页引导用户进入商城或提交咨询。
